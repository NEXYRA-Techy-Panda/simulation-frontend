...
test("4b. Every device retains bounded history when selection changes", () => {
  ...
  const first = createMockState({
    seq: 1,
    devices: [
      { device_id: "dev-a", room_id: "room-meeting", on: true, power_w: 10, override: null, energy_kwh: 1 },
      { device_id: "dev-b", room_id: "room-meeting", on: true, power_w: 20, override: null, energy_kwh: 2 },
    ],
  });
  ingestSimState(buffer, first, false, "room-meeting", "dev-a");
  const second = createMockState({
    seq: 2,
    sim_time_utc: "2026-01-01T04:30:10Z",
    devices: [
      { device_id: "dev-a", room_id: "room-meeting", on: true, power_w: 11, override: null, energy_kwh: 1.1 },
      { device_id: "dev-b", room_id: "room-meeting", on: true, power_w: 20, override: null, energy_kwh: 2 },
    ],
  });
  ingestSimState(buffer, second, false, "room-meeting", "dev-b");
  assert.equal(getScopeSeries(buffer, "device", "dev-a")?.samples.length, 2);
  assert.equal(getScopeSeries(buffer, "device", "dev-b")?.samples.length, 2);
  assert.equal(getScopeSeries(buffer, "device", "dev-a")?.latestPowerW, 11);
});

test("4c. Missing unselected devices become null gaps and energy is not clamped", () => {
  const buffer = createTelemetryBuffer(10);
  const first = createMockState({
    seq: 1,
    devices: [
  ...
  ingestSimState(buffer, first);
  const second = createMockState({
    seq: 2,
    sim_time_utc: "2026-01-01T04:30:10Z",
    devices: [
      { device_id: "dev-a", room_id: "room-meeting", on: true, power_w: 10, override: null, energy_kwh: 0.5 },
    ],
    office: { power_w: 10, energy_kwh: 0.5 },
  });
  ingestSimState(buffer, second);
  const a = getScopeSeries(buffer, "device", "dev-a");
  const b = getScopeSeries(buffer, "device", "dev-b");
  assert.equal(a?.latestEnergyKwh, 0.5);
  assert.equal(b?.samples.at(-1)?.power_w, null);
  assert.equal(b?.hasGaps, true);
});
...
test("5. Null vs zero: missing readings remain null (gaps), never zero", () => {
  ...
  ingestSimState(
    ...
    createMockState({
      seq: 3,
      sim_time_utc: "2026-01-01T04:30:20Z",
      office: { power_w: 100, energy_kwh: null },
    })
  );
  series = getScopeSeries(buffer, "office", "office");
  assert.equal(series?.hasGaps, true);
  assert.equal(series?.minPowerW, 100);
});
