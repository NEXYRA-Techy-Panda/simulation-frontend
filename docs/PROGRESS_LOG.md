...
## 2026-09-25 (IST) - K004-PREP3 completed (actual, Agent K-B - FreeBuff)

- Layer ID: K004-PREP3 (frontend environment controls UI preparation; the released K004 environment feature is NOT claimed). Owner: Kishore Kumar. UI preparation on an isolated branch only: no merge, no push, no deployment, nothing mounted.
- Isolation: worktree `simulation-frontend-k004`, branch `kishore/k004-environment-ui`, base `dbcbee9b935a3f6777f7a8bfca097d258e02e652` (== committed `main` == `origin/main`; no rebase/merge/cherry-pick/base update). `npm ci` was run inside this worktree only and the lockfile is unchanged. The main `simulation-frontend` working copy (K-A's uncommitted K003 export work: `sim-live.tsx`, `package.json`, new export files) was neither modified, stashed, reset, cleaned nor switched, and none of its files were copied here. Auditor repos, the Python service, the parent folder and production were not touched.
- Created `app/lib/environment.ts` (typed adapter for `POST /api/v1/environment` plus the controls' state machine): exact body `{ room_id, temp_c, rh_pct }` (all three required, nothing else); existing envelope; every acknowledgement field required with `applies_from === "next_step"`; 400/404/409 plus TIMEOUT/ABORTED/UNREACHABLE mapped honestly; bounded 15 s timeout; base URL through the existing `sanitizeOrigin` so the public `/sim` prefix is preserved; no new env var, domain or dependency. Bounds −30…60 °C / 0…100 %: blank, non-finite and out-of-range input rejected with per-field messages and never clamped; zero is valid. Nothing is computed in the browser (no power, energy or readings) and no seq/timestamp/climate is ever invented.
- Created `app/components/environment-controls.tsx`: reusable, explicitly NOT mounted anywhere (the main page, map, export controls and live-state coordinator were deliberately left to K-A/K003). Parent-owned props `backendUrl`, `selectedRoom`, `runId`, `acPowerModel`, `confirmedClimate`, `stale`, plus `fetchImpl`, `timeoutMs`, `onApplied`, `refreshConfirmed`; transient form state only. Apply is disabled with no run, no `/api/v1/state` model capability (missing/unknown model ⇒ unsupported, never assumed), a stale connection, or a pending request. Legacy runs are explained and never reset. Real labels, `aria-invalid`/`aria-describedby`, `role=status`/`role=alert` with `aria-live`, visible focus rings, and a stack/wrap layout that fits 375 px. Copy distinguishes backend-confirmed climate, unsaved input and an accepted command applying from the next simulated step, states that prescribed values are not sensor measurements, and explains that humidity is recorded but does not affect this demo's AC power model.
- Race safety: duplicate submissions refused; abort on unmount; epoch guard drops late responses across room/run changes including A → B → A so an old response cannot overwrite another room's state; entered values survive a failed submission; no automatic retry; timeouts reported as uncertain with refresh-before-retry guidance; confirmed success triggers the parent's authoritative refresh (`onApplied`) without fabricating readings.
- Created `app/lib/__tests__/environment-controls.test.mjs` (25 checks) and updated only the `package.json` test script. Checks: exact URL/body/envelope through a real temp-port HTTP server, the `/sim` prefix path, bare body vs envelope, every required field, blank vs zero, bounds without clamping, 400/404/409 mapping, timeout uncertainty, duplicate prevention, room/run change and A → B → A drops, no-run/legacy gating, success refresh without invented readings, `dispose()` aborting in flight work, and the controller's store seams. Backend-shaped fixtures live in tests only.
- Mid-task correction (recorded honestly): the first implementation passed tests/typecheck/build but failed `npm run lint` with 6 errors in `environment-controls.tsx` (4x `react-hooks/refs` ref access during render, 1x ref passed to a function through the `useState` initializer, 1x `react-hooks/set-state-in-effect`). Fixed by making the controller a real external store (cached immutable snapshot, stable identity between changes, `subscribe`, and `configure` for live handlers) consumed with `useSyncExternalStore`, with no ref reads during render. Result: 0 lint errors.
- Verification (final tree, temporary preview removed): `npm test` **44/44 pass** (19 committed `sim-state` + 25 new environment) exit 0; `npm run typecheck` clean exit 0; `npm run lint` **0 errors / 1 warning** — the pre-existing `scripts/verify-contract.mjs:188 'v' is assigned a value but never used`, confirmed present on the base commit and left untouched; `npm run build` exit 0 (`/` and `/_not-found` static). Temporary task-owned preview `app/environment-preview/page.tsx` (injected fetch returning 503, never contacting the public simulator) was served with `next start -p 3123` and returned HTTP 200 (16,969 bytes) with the supported/legacy/no-run states rendered and all Apply buttons disabled; the route was then deleted before commit and the full check set re-run, and the process was stopped with port 3123 verified free.
- Not performed, deliberately: the backend was not started, its 89-test suite was not repeated, `verify:contract` was not re-run (no contract file changed), and nothing was pushed, merged, deployed or deleted. Contract **1.0.1 unchanged**; the AC power model identity stays internal and no comparison claim is added.
- Limitations: **no browser was available in this session**, so Apply clicks, focus order, live-region announcements, hydration-time capability notices and the real 375 px layout were not verified, and **no end-to-end integration was exercised** — the mocked SSR preview must not be called real backend integration. Feature-branch deployment behaviour remains unconfirmed (no CI config or deploy script; `origin` has only `main`).
- Backend dependency: `../simulation-backend` branch `kishore/k004-environment-prep`, commits `43aa9fa` (K004-PREP) and `13d59b6` (K004-PREP2) — preserved, unedited during this task, unmerged, unpushed. The controls target that branch's actual implementation.
- Files changed: created `app/lib/environment.ts`, `app/components/environment-controls.tsx`, `app/lib/__tests__/environment-controls.test.mjs`, `docs/K004_ENVIRONMENT_UI_PREP_EVIDENCE.md`; updated `package.json` (test script only), `docs/ACTIVE_TASK.md`, `docs/HANDOFF.md`, this log. Excluded from the commit: the temporary preview route, `node_modules`, `.next`.
