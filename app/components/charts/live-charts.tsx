...
export default function LiveCharts({
  ...
  // Persistent bounded buffer in component state
  ...
  const [prevSampleKey, setPrevSampleKey] = useState<string>("");

  // Synchronize incoming state sample during render (React 19 recommended pattern for props-derived state)
  const currentKey = `${state?.run_id ?? ""}:${state?.seq ?? ""}:${state?.sim_time_utc ?? ""}:${isStale}:${selectedRoom?.room_id ?? ""}:${selectedDevice?.device_id ?? ""}`;
  if (state && currentKey !== prevSampleKey) {
    setPrevSampleKey(currentKey);
    ingestSimState(
      buffer,
      state,
      isStale,
